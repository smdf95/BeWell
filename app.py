...
from forms import MoodForm, Relief, GratitudeForm
...
def index():
    ...
    if form.validate_on_submit():
        if mood == "Awful" or mood == "Not Good" or mood == "Okay":
            return redirect(url_for("relief"))
        else:
            return redirect(url_for("gratitude"))
    return render_template("index.html", form=form, quote=quote, author=author)
...
def gratitude():
    ...
    if form.validate_on_submit():
        gratitude_journal.append({"date": date.today(), "name": form.name.data + ":", "gratitude": form.gratitude.data})
        return redirect(url_for("gratitude"))
